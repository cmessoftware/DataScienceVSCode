...
class ChurnPredictor:
    ...
    if "Churn" in df_improvement.columns:
        df_improvement = df_improvement.drop("Churn", axis=1)
    ...
    if "MonthlyCharges" in df.columns and "TotalCharges" in df.columns:
        ...
        df_improvement["Charge_Per_Month_Ratio"] = df_improvement["TotalCharges"] / (
            df_improvement["tenure"] + 1
        )
        df_improvement["High_Monthly_Charges"] = (
            df_improvement["MonthlyCharges"] > df_improvement["MonthlyCharges"].quantile(0.75)
        ).astype(int)
        df_improvement["High_Total_Charges"] = (
            df_improvement["TotalCharges"] > df_improvement["TotalCharges"].quantile(0.75)
        ).astype(int)
    ...
    if available_services:
        ...
        for col in available_services:
            if col == "PhoneService":
                df_improvement["Total_Services"] += (df_improvement[col] == "Yes").astype(int)
            elif col == "InternetService":
                df_improvement["Total_Services"] += (df_improvement[col] != "No").astype(int)
            else:
                df_improvement["Total_Services"] += (df_improvement[col] == "Yes").astype(int)
    ...
    if "SeniorCitizen" in df.columns:
        df_improvement["SeniorCitizen_Int"] = df_improvement["SeniorCitizen"].astype(int)
    ...
    if "Contract" in df.columns:
        ...
        df_improvement["Long_Contract"] = (df_improvement["Contract"] == "Two year").astype(
            int
        )
