...
# -*- coding: utf-8 -*-
"""
Módulo de Modelos para Predicción de Churn - Versión Estable
...
class ChurnPredictor:
    """
    Predictor de Churn - Versión Estable
    ...
    def __init__(self, random_state=42):
        ...
        print(f"🗺️  ChurnPredictor inicializado con random_state={random_state}")
    ...
    def inspect_transformed_columns(
        ...
        if not current_preprocessor is None:
    ...
    def create_preprocessor(self, X_train):
        ...
        if "Churn" in X_clean.columns:
            X_clean = X_clean.drop("Churn", axis=1)
    ...
    def create_models(self, X: pd.DataFrame):
        ...
        preprocessors = {
            ...
            "Gradient_Boosting": self._pick_preprocessor('Gradient_Boosting',X_for_schema)
        }
        ...
        print(f"✅ Preprocessors were configured successfully")
                ...
                [
                    ...
                    (
                        ...
                        LogisticRegression(
                            ...
                            max_iter=1000,
                            class_weight="balanced",  # MEJORA: Balanceo de clases --> IMPORTANTE PARA TELCO CHURN que tiene alto desbalanceo entre No Churn / Churn
                        ),
                    ),
                ]
            ),
    ...
    def generate_model_report(self, X_test, y_test):
        ...
        for name, model in self.models.items():
            try:
                ...
                f1 = f1_score(y_test, y_pred, average="weighted", zero_division=0)
                roc_auc = roc_auc_score(y_test, y_pred_proba,name)
                ...
                # Matriz de confusión
                show_confusion_matrix(y_test,y_pred,name):
...
def hyperparameter_tuning(model, param_grid, X_train, y_train, cv=5, scoring="roc_auc"):
    """
    Optimización de hiperparámetros (robusta):
    - Valida claves de grilla
    - CV estratificado
    - error_score=np.nan (omite combos inválidos)
    """
    print("🗺️  Iniciando optimización de hiperparámetros...")
    print(f"🗺️  CV folds: {cv}, Scoring: {scoring}")

    # 1) Validar claves de la grilla
    valid_keys = set(model.get_params().keys())
    unknown = [k for k in param_grid.keys() if k not in valid_keys]
    if unknown:
        print("❌ Claves inválidas en la grilla:")
        for k in unknown:
            print(f"   - {k}")
        print("👉 Usá nombres completos del pipeline (ej.: 'classifier__C').")
        return None

    # 2) Simplificar (si querés mantener tu lógica)
    simple_grid = {}
    param_count = 0
    for key, values in param_grid.items():
        if isinstance(values, list) and len(values) > 6:
            simple_grid[key] = values[:6]  # un poco más amplio
        else:
            simple_grid[key] = values
        if isinstance(simple_grid[key], list):
            param_count += len(simple_grid[key])
        else:
            param_count += 1
    print(f"🗺️  Grilla (conteo aprox valores): {param_count}")

    # 3) CV estratificado
    cv_splitter = StratifiedKFold(n_splits=cv, shuffle=True, random_state=42)

    # 4) GridSearchCV tolerante a errores
    grid_search = GridSearchCV(
        ...
        error_score=np.nan,  # ⬅️ clave: no aborta por combos inválidos
    )

    print("🗺️  Ejecutando búsqueda en grilla...")
    ...
    # Verificar que haya resultados válidos
    if (
        not hasattr(grid_search, "cv_results_")
        or len(grid_search.cv_results_["mean_test_score"]) == 0
    ):
        print("⚠️ La búsqueda no produjo resultados.")
        return None

    # Si todas las combinaciones dieron NaN, best_score_ puede fallar
    try:
        print(f"✅ Mejor score: {grid_search.best_score_:.4f}")
        print(f"✅ Mejores parámetros: {grid_search.best_params_}")
        print("✅ Optimización completada")
    except Exception as e:
        print(f"⚠️ No se pudo determinar best_score_/best_params_: {e}")
        # Mostrar top filas de resultados por si sirve
        df = pd.DataFrame(grid_search.cv_results_)
        cols = [c for c in df.columns if c.startswith("mean_test_")] + ["params"]
        print(df[cols].head())
        return None
...
def crear_caracteristicas_avanzadas(df):
    """
    MEJORA: Función de ingeniería de características avanzada para churn prediction
    ...
    # Eliminar columna Churn si existe (para evitar data leakage en test)
    if "Churn" in df_mejorado.columns:
        df_mejorado = df_mejorado.drop("Churn", axis=1)

    # Características financieras
    if "MonthlyCharges" in df.columns and "TotalCharges" in df.columns:
        # Convertir TotalCharges a numérico
        df_mejorado["TotalCharges"] = pd.to_numeric(
            df_mejorado["TotalCharges"], errors="coerce"
        )
        df_mejorado["TotalCharges"] = df_mejorado["TotalCharges"].fillna(
            df_mejorado["TotalCharges"].median()
        )

        # Nuevas características financieras
        df_mejorado["Charge_Per_Month_Ratio"] = df_mejorado["TotalCharges"] / (
            df_mejorado["tenure"] + 1
        )
        df_mejorado["High_Monthly_Charges"] = (
            df_mejorado["MonthlyCharges"] > df_mejorado["MonthlyCharges"].quantile(0.75)
        ).astype(int)
        df_mejorado["High_Total_Charges"] = (
            df_mejorado["TotalCharges"] > df_mejorado["TotalCharges"].quantile(0.75)
        ).astype(int)
    ...
    if available_services:
        ...
        df_mejorado["Total_Services"] = 0
        for col in available_services:
            if col == "PhoneService":
                df_mejorado["Total_Services"] += (df_mejorado[col] == "Yes").astype(int)
            elif col == "InternetService":
                df_mejorado["Total_Services"] += (df_mejorado[col] != "No").astype(int)
            else:
                df_mejorado["Total_Services"] += (df_mejorado[col] == "Yes").astype(int)
        ...
        available_protection = [col for col in protection_services if col in df.columns]
        if available_protection:
            df_mejorado["Protection_Services"] = 0
            for col in available_protection:
                df_mejorado["Protection_Services"] += (
                    df_mejorado[col] == "Yes"
                ).astype(int)

    # Características demográficas
    if "SeniorCitizen" in df.columns:
        df_mejorado["SeniorCitizen_Int"] = df_mejorado["SeniorCitizen"].astype(int)

    # Características de contrato
    if "Contract" in df.columns:
        df_mejorado["Short_Contract"] = (
            df_mejorado["Contract"] == "Month-to-month"
        ).astype(int)
        df_mejorado["Long_Contract"] = (df_mejorado["Contract"] == "Two year").astype(
            int
        )

    if "PaymentMethod" in df.columns:
        df_mejorado["Electronic_Payment"] = (
            df_mejorado["PaymentMethod"] == "Electronic check"
        ).astype(int)
