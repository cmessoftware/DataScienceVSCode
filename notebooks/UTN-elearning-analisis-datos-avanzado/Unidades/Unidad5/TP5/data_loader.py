...
def load_competition_data(data_dir='.'):
    ...
    try:
        ...
        print("💡 Asegúrate de que los archivos train.csv, test.csv y sample_submission.csv estén en el directorio especificado")
...
def create_sample_data(n_train=1000, n_test=200, random_state=42):
    ...
    train_df = pd.DataFrame({
        ...
        'MultipleLines': np.random.choice(['Yes', 'No', 'No phone service'], n_train),
        'InternetService': np.random.choice(['DSL', 'Fiber optic', 'No'], n_train, p=[0.35, 0.45, 0.2]),
        'OnlineSecurity': np.random.choice(['Yes', 'No', 'No internet service'], n_train),
        'OnlineBackup': np.random.choice(['Yes', 'No', 'No internet service'], n_train),
        'DeviceProtection': np.random.choice(['Yes', 'No', 'No internet service'], n_train),
        'TechSupport': np.random.choice(['Yes', 'No', 'No internet service'], n_train),
        'StreamingTV': np.random.choice(['Yes', 'No', 'No internet service'], n_train),
        'StreamingMovies': np.random.choice(['Yes', 'No', 'No internet service'], n_train),
        'Contract': np.random.choice(['Month-to-month', 'One year', 'Two year'], n_train, p=[0.55, 0.25, 0.2]),
        'PaperlessBilling': np.random.choice(['Yes', 'No'], n_train),
        'PaymentMethod': np.random.choice(['Electronic check', 'Mailed check', 'Bank transfer (automatic)', 'Credit card (automatic)'], n_train),
    ...
    contract_effect = np.where(train_df['Contract'] == 'Month-to-month', 0.3,
    ...
    test_df = train_df.drop('Churn', axis=1).sample(n_test, random_state=random_state).reset_index(drop=True)
    test_df['customerID'] = [f'T{i:04d}' for i in range(len(test_df))]
    
    # Simular archivo de submission
    sample_submission_df = pd.DataFrame({
        'customerID': test_df['customerID'],
        'Churn': 0.5  # Probabilidad base
    })
